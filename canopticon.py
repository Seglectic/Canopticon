...
def make_overlay(
    ...
    overlay[occluded_binary] = cv2.addWeighted(
        image_bgr[occluded_binary], 1.0 - alpha,
        red[occluded_binary], alpha,
        0.0,
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Batch canopy/sky occlusion estimation using skyseg ONNX."
    )
    parser.add_argument("input_dir", type=Path, help="Folder of input photos")
    parser.add_argument("output_dir", type=Path, help="Folder for outputs")
    parser.add_argument(
        "--model",
        type=str,
        default=None,
